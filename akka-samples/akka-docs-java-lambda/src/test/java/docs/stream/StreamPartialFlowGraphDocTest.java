...
public class StreamPartialFlowGraphDocTest {
  ...
  public void demonstrateBuildSourceFromPartialFlowGraph() throws Exception {
    ...
    final Source<Pair<Integer, Integer>> pairs = Source.fromGraph(builder -> {
      // connect the graph
      builder
        .addEdge(ints, Flow.<Integer>empty().filter(e -> e % 2 != 0), zip.left())
        .addEdge(ints, Flow.<Integer>empty().filter(e -> e % 2 == 0), zip.right())
    ...
    final Flow<Integer, Pair<Integer, String>> pairUpWithToString = 
      Flow.create(builder -> {
        ...
        builder
          ...
          .addEdge(broadcast, Flow.<Integer>empty().map(e -> e), zip.left())
          .addEdge(broadcast, Flow.<Integer>empty().map(e -> e.toString()), zip.right())
          .addEdge(zip.out(), undefinedSink);
